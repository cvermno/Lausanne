...
1. We adjusted the model parameters to improve performance. Let's quilcky define each of them:
   - MAX_LEN: Maximum number of words the model processes in a sentence. Longer sequences are truncated, shorter ones are padded.
   - BATCH_SIZE: Number of sentences processed simultaneously during training or evaluation. Balances memory use and training speed.
   - LEARNING_RATE: Step size for updating model parameters during optimization. Balances between convergence speed and training stability.
   - NUM_EPOCHS: Number of times the model iterates over the entire dataset. More epochs can improve learning but risk overfitting.
